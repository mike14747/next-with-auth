...
const getUserForSignin = async (username, password) => {
    ...
    if (!username || !pattern1.test(username)) return null;

    const pattern2 = new RegExp(passwordPattern);
    if (!password || !pattern2.test(password)) return null;
...
const changePassword = async (_id, password, resetPasswordToken = null) => {
    if (!_id || !ObjectId.isValid(_id)) return { code: 400 };
