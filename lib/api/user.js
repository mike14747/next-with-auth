...
const changePassword = async (_id, password, token = null) => {
    ...
    const user = await db
        .collection('users')
        .findOne({ _id: ObjectId(_id) }, { projection: { _id: 1, password: 1, salt: 1 } });
    if (!user) return { code: 500 };

    const hashedPassword = hashPassword(password, user.salt);
    if (!hashedPassword) return { code: 500 };
    if (hashedPassword === user.password) return { code: 200 };
    ...
    const updateResult = await db
        .collection('users')
        .updateOne({ _id: ObjectId(_id) }, { $set: { password: newHashedPassword, salt } });
