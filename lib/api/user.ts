...
export const forgotUsername = async (email: string) => {
    ...
    try {
        ...
        if (user.length === 0) return { code: 404 }; // email address doesn't match any in the database
...
export const resetPassword = async (username: string, email: string, baseUrl: string) => {
    ...
    try {
        ...
        if (!user) return { code: 500 }; // query failed
        if (user.length === 0) return { code: 404 }; // username/email combo doesn't match any in the database
...
export const deleteUserAccount = async (_id: string) => {
    ...
    try {
        ...
        if (!deleteResult) return { code: 500 };
        return deleteResult?.deletedCount === 1 ? { code: 200 } : { code: 404 };
    } catch (error) {
        console.log(error);
        return { code: 500 };
    }
